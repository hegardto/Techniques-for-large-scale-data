...
def assignPoints(worker_id, workers, N, data, centroids, c, cluster_sizes, variation):
    for i in range(int(worker_id*(N/workers)),int((worker_id+1)*(N/workers))):
...
def kmeans(workers, k, data, nr_iter = 100):
    ...
    for j in range(nr_iter):
        ...
        variation = mp.Array('f',range(k))
        cluster_sizes = mp.Array('i',range(k))
        ...
        for i in range(workers):
            process = mp.Process(target = assignPoints, args = (i, workers, N, data, centroids, c, cluster_sizes, variation))
        ...
        centroids = centroids / np.reshape(cluster_sizes,(-1,1))
...
def measure_time():
    ...
    times = []
    speedup = []
    theoretical_speedup = [0.95 * x for x in cores]
    
    for k in cores:
        times.append(kmeansTime(k))
    ...
    plt.plot(cores, theoretical_speedup, color = 'orange', label = "Theoretical speedup")
